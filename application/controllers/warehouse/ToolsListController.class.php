    ...
    public function index()
    {
      ...
      if (isset($_GET['selectSize'])) {
       ...
       //联合查询：查询设备列表及对应的分类
        ...
        //获取施工人员，提供给负责人做下拉菜单
        $mysqlModel_builders = new ToolsListModel("pmanage_builders");
        $conditions_m = "where  adminBumenId in ($myAdminBumensubArrString)";
        $pmanage_builders = $mysqlModel_builders ->selectAll($conditions_m);
        $pmanage_buildersJson = json_encode($pmanage_builders);
        $this->assign('pmanage_buildersJson',$pmanage_buildersJson);
    }
