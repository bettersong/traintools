...
class personToolsCheckInModel extends Model
{
    ...
    public function getSmallToolsDataFromToolbag(){
       ...
       foreach($res as $index => $value){
          ...
          //获取该小工具 在工具包中的真实数量 //FIND_IN_SET(str,strlist)函数 查询字段(strlist)中包含(str)的结果，返回结果为null或记录
      ...
      //echo '<br><br>';
       ...
       foreach($res as $key => $value){
            ...
            */
            ...
            $toolnums_list = 0;
            $bigToolsArr = array();
            //print_r($bigToolIdsTemp);
            foreach($bigToolIdsTemp as $key2=>$value2){
                //$toolsArr[$toListId]['name']
                $gpsCode = $bigToolGPSs[$key2]['GPSCode'];
                $bigToolsArr[$toListId]['details'][$key2]['gpsCode']=$gpsCode;
                //$bigToolsArr[$toListId]['details'][$key2]['toolId']=$value['toListId'];
                $bigToolsArr[$toListId]['details'][$key2]['toolName']=$value['toListName'];
               ...
               $devGPS_x = "";
               $devGPS_y = "";
               $bigToolsArr[$toListId]['details'][$key2]['status']= 0;//是否在范围内
               if($res2){//找到经纬度信息
                    ...
                    if($distance=="") $bigToolsArr[$toListId]['details'][$key2]['status']= 0;//未找到
                    else if( $distance>DistanceRange_safeDoor){
						$bigToolsArr[$toListId]['details'][$key2]['status']= 2; //超出范围
					}
					else {//DistanceRange_safeDoor米以内
                        $bigToolsArr[$toListId]['details'][$key2]['status']= 1;//定位设备找到，且在x米范围内。
                        $toolnums_list ++;
                    }
               }
               else{//为找到实时经纬度信息
               ...
               $bigToolsArr[$toListId]['details'][$key2]['devGPS_x']= $devGPS_x;
               $bigToolsArr[$toListId]['details'][$key2]['devGPS_y']= $devGPS_y;
               $bigToolsArr[$toListId]['details'][$key2]['distance']= $distance;
            ...
            $bigToolsArr[$toListId]['thInfo']['safaDoorinName'] = $safaDoorinName;
            $bigToolsArr[$toListId]['thInfo']['toListId'] = $value['toListId'];
            $bigToolsArr[$toListId]['thInfo']['name'] = $value['toListName'];
            $bigToolsArr[$toListId]['thInfo']['twtlPreparedAmount'] = $value['twtlPreparedAmount'];//班前数量
            $bigToolsArr[$toListId]['thInfo']['twtlRealAmount'] = $toolnums_list;
            $bigToolsArr[$toListId]['thInfo']['amountDiffer'] = $value['twtlPreparedAmount'] - $toolnums_list;
             //echo '  len='.count($bigToolsArr).'  ';
            //工具集合对应的定位器集合
            //$bigToolIds = $value['twtlDetail'];
            //$bigToolsArr =  $bigToolsArr =  explode(",", $bigToolIds);;
            
             //echo   '<br>'; print_r($bigToolsArr);
       }
       //print_r($bigToolsArr);
       return $bigToolsArr;
