...
<div id="header" class="navbar navbar-inverse navbar-fixed-top" style="position:fixed">
  ...
  <div class="navbar-inner">
    <div class="container-fluid">
      ...
      <h4>高铁线路施工人员和工器具上线管理系统 <?='level='.$_SESSION['userInfo']['level']?><?php /*?><span style="font-size:14px;color:#FFFF00;">（&nbsp;<?=$_SESSION['userInfo']['pManageName']?>: <?=$_SESSION['userInfo']['bumenTree']?>&nbsp;）</span><?php */?></h4> 
...
<div class="sidebar-scroll">
  <div id="sidebar" class="nav-collapse collapse">
    ...
    <ul class="sidebar-menu" id="leftSidebar">
     <li class="sub-menu onlyitem1"> <a class="level1a" href="/index/index"><span>工作面板</span><span class="arrow"></span></a> </li>
      <?php 
	  if($level_showItem[$_SESSION['userInfo']['level']]=="")$level_showItem[$_SESSION['userInfo']['level']]=$level_showItem[-1];
	  $i=0;
	  foreach ($catlogForLeft as $key => $value) { //遍历输出左侧导航
	     //不同level的部门对于不同显示项，在config中有配置
		 $levelArr = $level_showItem[$_SESSION['userInfo']['level']];
		 if(strpos($_SESSION['userInfo']['bManageName'],'段领导') !== false){
			 $levelArr = $level_showItem[4-1];
		 }
	     if ( !in_array($key, $levelArr) ){
		  continue;
		  }
		  $i++;
	  ?>
            <li class="sub-menu <?php if(!checkAuthByAction("show",$key,"",""))echo 'noauth';?>"> <a class="level1a" href="javascript:;" class=""> <span><?=$value['name']?></span> <span class="arrow"></span> </a>
            <ul class="sub" style="<?php if(count($level_showItem[$_SESSION['userInfo']['level']])<3)echo 'display:block;'?>">
              <?php //遍历输出二级导航
			   if($value['subnav'] != ""){
