                ...
                <div class="span12">
                    ...
                    <div class="widget red">
                        ...
                        <div class="widget-body">
                            <div>
                               <div>
                               	<img style="width:13px;" src="/public/images/icon-png/bumen.png">表示部门(可添加子项) &nbsp;
                                <!--<img  style="width:13px;vertical-align:text-top;margin-top: 2px;" src="/public/images/icon-png/zhiwei.png">表示职位-->
                                </div>
...
$(function () {
	...
	$(".addchild").live('click',function(e) {
		...
		if( $(this).parent("li").eq(0).children("#ul-bumen"+bumenId).length>0 ){//子菜单添加
			...
			//'<label><input name="addtype" type="radio" value="1" />部门</label>'+
			//'<label><input name="addtype" type="radio" value="0" />职位</label>'+
	...
	$(".addBtn").live('click',function(e) {
		...
		var level = $(this).prevAll("input").attr("level");
 		var addtype = 1;//默认为部门。$("input[name='addtype']:checked").val();
		...
		/*else if(typeof(addtype)=="undefined" || addtype==""){
			layer.alert("您添加的是部门还是职位，请选择！", {icon:0,title: '【提示】'});
			return false;
		}*/
		if(pid=="")return false;
		//alert(name+" "+pid);
		...
		//alert("addtype:"+addtype+"  name:"+name+"  pid:"+pid+"  level:"+level);
		//AJAX提交到后台
		$.ajax({
        async:false,
        type: "post",
	...
	$(".editItem").live('click',function(e) {
		...
		//if(bumenOrZhiwei==1)inputhtml += '<label><input checked name="addtype" type="radio" value="1" />部门</label>'+'<label><input name="addtype" type="radio" value="0" />职位</label>';
		//else inputhtml += '<label><input name="addtype" type="radio" value="1" />部门</label>'+'<label><input checked name="addtype" type="radio" value="0" />职位</label>';
	...
	$("#treeDiv font").hover(function(){ //.live('hover',function(e) {
		...
		if(bumenorzhiwei==1){
			...
			if( $(this).attr("isAdministration")==1 ){//已经是主
				...
				$(this).after('<input  class="setAdminBumen"  type="button" value="设为主管单位" />');
			}
			
		}
		var timeout_setadmin=setTimeout('$(".setAdminBumen").remove()',5000);
		...
		for (var i in childArry) {
			...
			if(isSuperAdmin || myAdministration){
				...
				if(isSuperAdmin || k==0 || (k>0 && isInArray(myTopBumenArr,childArry[i].pid)) ){
					...
					if(childArry[i].bumenOrZhiwei==1)menus += '<span title="添加子菜单" class="addchild" bumenId="'+childArry[i].id+'"   level="'+childArry[i].level+'" pid="'+childArry[i].pid+'">添</span>';//只有部门下可以添加子项，职位为最低级
