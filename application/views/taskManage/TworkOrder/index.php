            ...
            <div class="row-fluid">
                <div class="span12">
                    <h3 class="page-title" style="color:#000">
                        <?php  
                               if($_GET['pManage']==1)echo "施工上线人员管理"; // 施工上线人员管理
                               else if($date==date('Y-m-d')) echo "今日工单"; 
                               else echo "历史工单";
                            ?>
...
<div class="notPmanage" style="border: 1px solid #ddd;margin: 0 0 20px;padding: 3px 0;">
    <div style="color:red;font-size:1.1em;padding:2px 0 3px 5px;">今日全部工单（点击查看）：</div>
    <?php foreach ($TworkOrderALL as $key => $value) {?>
        <div style="border-top:1px solid #ddd;padding:0 0 0 5px;"><a class="<?php if($_GET['ordernum']==$key)echo 'currenOrdernum';else echo 'ordernum';?>" style="color:#094;" href="/taskManage/TworkOrder/index&ordernum=<?=$key?>">
        <span style="font-weight:600;">工单<?=$TworkOrder['twOrderId']?></span>——<?=$value['QiQiSJ']?> >> <span><?=$value['ZuoYeXM']?></span></a></div>
    <?php } ?>
