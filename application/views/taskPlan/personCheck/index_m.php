...
<html lang="en" class="feedback">
	<head>
		...
		.mainGps img, .gps img,.Gps img{width:16px;    margin-top: -3px;}
	    ...
	    li.phone{width:98px;}
		li.has a{padding:0 10px 0 0;}
		li.has a{background: url(/public/images/phone2.png) no-repeat right/12px 12px}
		.name .updateObj{display:inline-block;width:40px;text-align: right;}
		</style>
	</head>
	...
	<body>
					...
					<div class="mui-scroll" style="padding:20px 0px 0px 5px">
						...
						<ul class="mui-table-view mui-table-view-chevron list">
							...
							<li class="name" value=<?=$value['twamPersonId']?> twamId=<?=$value['twamId']?> >
								<?php if($value['twamName']=="")echo '<span class="updateObj noSelectedPerson">无'; else echo '<span class="updateObj">'.$value['twamName'];?></span><img src="/public/images/edit.png?a2" />
							</li>
							<?php } else{//负责人不可编辑?>
							<li class="unedit" value="" twamId=<?=$value['twamId']?> >
								<span class="updateObj"><?php if($value['twamName']=="")echo "无";else echo $value['twamName'];?></span>
							</li>
							<?php }?>
							<li class="phone <?=$value['pManageContact']==''?'null':'has'?>" style="width: ;text-align:left;"><a href="tel:<?=$value['pManageContact']?>"><?=$value['pManageContact']?></a></li>
						...
						<ul class="mui-table-view mui-table-view-chevron list shigongList">
							...
							<li class="name" value=<?=$value['twkePersonId']?> twamId=<?=$value['twkeId']?> >
								<?php if($value['pManageName']=="")echo '<span class="updateObj noSelectedPerson">无'; else echo '<span class="updateObj">'.$value['pManageName'];?></span><img src="/public/images/edit.png?a2" />
							</li>							
							<li class="phone <?=$value['pManageContact']==''?'null':'has'?>" style="width: ;text-align:left;"><a href="tel:<?=$value['pManageContact']?>"><?=$value['pManageContact']?></a></li>
						...
						<button type="button" id="submit_confirm" class="mui-btn mui-btn-success submit">确认准备完毕</button>
					</div>
		</div>
		</div>	
		</div>
		<div class="<?php if( $orderInfoArr['twOrderConfirmed_persons']==1) echo 'confirmedTools';?>" title="已确认完毕">&nbsp;</div>
		...
		<script type="text/javascript">
                	...
                	if(e.index == 1){
						...
						.done(function(msg) {
							...
							var newList = '<ul class="mui-table-view mui-table-view-chevron list shigongList">'+'<li class="name" value="" twamId=msg["pManageId"] >'+
								'<span>'+msg["pManageName"]+'</span>'+'<img src="/public/images/edit.png" /></li>'+'<li class="phone" style="width: 90px;text-align:left;">'+msg["pManageContact"]+'</li>'+'<li class="Gps" value='+msg["GPSId"]+'twamId='+msg["tid"]+'>'+GPSCode+'<img src="/public/images/pulldown.png"></li>'+'<li class="delete deleteBuilder" tid='+msg['tid']+'><img src="/public/images/delete.png"></li>';
			...
			$('#ulBox').on('click','.name',function(){
				...
				if(obj_type == "builders"){
					var pManageId = $(this).val();
					var pManageName = $(this).find('.updateObj').html();
					newArry = buildersAllJson;
					personOption += '<option selected value="'+pManageId+'">'+pManageName+'</option>';
				}

				console.log(buildersAllJson);
				personOption += '<option value=""></option>';
			...
			$('#ulBox').on('click','.Gps',function(){
				...
				if(gpsCode==undefined){
				gpsBoxContent += '<option selected value="">'+'无'+'</option>';	
				}else{
