    ...
    return {
        ...
        init: function () {
            ...
            function editRow(oTable, nRow,actionType="edit"){
                ...
                //遍历后台获取的类别信息
                var devOption = "";
                $.each(bManageArray,function(index,row){
                    //alert(device_id);
                     devOption += '<option value="'+row['bManageId']+'">'+row['bManageBranch']+'</option>'; //构造option

                });
                if(actionType=="add")
                jqTds[1].innerHTML = '<select class="bumen" id="select">'+devOption+'</select>';
                else jqTds[1].innerHTML = '<select class="bumen" id="select">'+devOption+'</select>';
