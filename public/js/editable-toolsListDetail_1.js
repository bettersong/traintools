    ...
    return {
        ...
        init: function () {
            ...
            function editRow(oTable, nRow,actionType="edit") {
                ...
                //遍历后台获取的类别或定位器编码
                var devOption = "";
                var RFIDTagCodeArr = aData[2].split(",");
                var j = 1;
                $.each(rfidtagArray,function(index,row){
                    var sign = 0 ;//遍历标记
                    var splitTag = ',';
                    for (var i = 0; i < RFIDTagCodeArr.length; i++) {
                        if(RFIDTagCodeArr[i]==row['RFIDTagCode'])
                        {
                            devOption += '<option selected="selected" value="'+row['RFIDTagId']+'">'+row['RFIDTagCode']+splitTag+'</option>';
                            sign = 1;
                        }
                    }
                    if (sign == 0) {devOption += '<option value="'+row['RFIDTagId']+'">'+row['RFIDTagCode']+splitTag+'</option>';}
                  j++;    
               });
               jqTds[2].innerHTML = '<select multiple="multiple" class="toListRFIDCode" id="select2">'+devOption+'</select>';
            ...
            function cancelEditRow(oTable, nRow) {
                var jqInputs = $('input', nRow);
                var b = $('#select2 option:selected',nRow).text();
                var d = $('#select1 option:selected',nRow).text();
                oTable.fnUpdate(jqInputs[0].value, nRow, 0, false);
                oTable.fnUpdate(jqInputs[1].value, nRow, 1, false);
                oTable.fnUpdate(b, nRow, 2, false);
                oTable.fnUpdate('<a class="edit" href="">编辑</a>', nRow, 3, false);
