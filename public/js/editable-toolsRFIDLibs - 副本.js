    ...
    return {
        ...
        init: function () {
            ...
            function editRow(oTable, nRow,actionType="edit") {
                ...
                //遍历后台获取的类别信息
                var devOption = "";
                $.each(RFIDLibsArray,function(index,row){
                    //alert(device_id);
                     devOption += '<option value="'+row['RFIDClassId']+'">'+row['RFIDClassType']+'</option>'; //构造option
                });
                if(actionType=="add")
                jqTds[1].innerHTML = '<select class="name" id="select">'+devOption+'</select>';
                else jqTds[1].innerHTML = '<select class="name"  id="select">'+devOption+'</select>';
                jqTds[2].innerHTML = '<input type="text" class="code small" value="' + aData[2] + '">';
                jqTds[3].innerHTML = '<input type="text" class="type small" value="' + aData[3] + '">';
                //jqTds[3].innerHTML = '<input type="text" class="time small" value="' + aData[3] + '">';
            ...
            function saveRow(oTable, nRow) {
                ...
                oTable.fnUpdate(tmp, nRow, 3, false);
