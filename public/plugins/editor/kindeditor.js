...
K.options = {
	...
	items : [
		...
		'anchor', 'link', 'unlink', '|', 'clearhtml'//, 'about'
	],
...
function _mediaEmbed(attrs) {
	...
	html += '></video>';*/
	var html = '<div class="videoBox"><video class="videoPlayer"  src="'+attrs.src+'" controls="controls" preload="auto"></video></div>';
	return html;
}
var i=1;
function _mediaImg(blankPath, zdy,attrs) {
	//alert(" attrs-width="+i);i++;
	var width = attrs.width;
	var height = attrs.height;
	var type = attrs.type || _mediaType(attrs.src);
	var srcTag = _mediaEmbed(attrs);
	var style = '';
	if(zdy=='zdy'){ //视频
	    //alert(attrs.width+"  srcTag="+srcTag);
		var html=srcTag;
	}else{
		if (width > 0) {
	...
	var html=srcTag;
		...
		var html = '<img class="' + _mediaClass(type) + '" src="' + blankPath + '" ';
		if (style !== '') {
			html += 'style="' + style + '" ';}html += 'data-ke-tag="' + escape(srcTag) + '" alt="" />';
		}
	return html;*/
...
_extend(KEdit, KWidget, {
	init : function(options) {
		...
		self.iframe = K('<iframe id="editbox" name="editbox" class="ke-edit-iframe" hidefocus="true" frameborder="0"' + iframeSrc + '></iframe>').css('width', '100%');
...
KEditor.prototype = {
	...
	loadPlugin : function(name, fn) {
		...
		self.isLoading = true;
		_loadScript(self.pluginsPath + name + '/' + name + '.js?ver=a1' + encodeURIComponent(K.DEBUG ? _TIME : _VERSION), function() {
			self.isLoading = false;
			if (_plugins[name]) {
